...
/**
 * Bag whose structure is complete and whose contents have been validated. Once a <code>Bag</code> is validated, it
 * can't be changed. If you want to make changes to it, you must clone it and make changes there. Otherwise, the bag
 * is ready for packaging and transmission. <div>A <code>ValidBag</code> is constructed using a
 * <code>BagValidator</code> . For instance: <blockquote>
 * <code>Bag bag = new Bag("papas_got_a_brand_new").complete();</code>
 * <code>ValidBag validBag = new BagValidator().validate(bag);</code> </blockquote> A validated bag can then be used
 * to create a bag file that is ready for transmission.</div>
 */
public class ValidBag {

    private final Bag myBag;

    ValidBag(final Bag aBag) {
        aBag.validate();
        myBag = aBag;
    }
